    ...
    <>
      {mounted && (
        <>
          ...
          {/* Radio popup — always in DOM so audio persists, shown/hidden via opacity */}
          <div
            ...
            style={{
              ...
              transform: radioOpen ? 'translateY(0)' : 'translateY(8px)',
              transition: 'opacity 0.2s ease, transform 0.2s ease',
              pointerEvents: radioOpen ? 'auto' : 'none',
            }}
          >
            {/* Branding strip */}
            <div style={{ padding: '9px 13px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '0.5px solid #181818' }}>
            ...
            <div style={{ padding: '13px 13px 10px', display: 'flex', gap: '11px', alignItems: 'center' }}>
              <div style={{
                ...
                border: '2px solid #1c1c1c', position: 'relative', flexShrink: 0,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {hasTracks ? (
                  <>
                    <div style={{ color: '#e0e0e0', fontSize: '12px', fontWeight: '500', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '4px' }}>
            ...
            <div style={{ padding: '0 13px', marginBottom: hovering ? '12px' : '10px' }}>
              <div style={{ height: '2px', background: '#1c1c1c', borderRadius: '2px' }}>
                <div style={{ width: `${progress}%`, height: '100%', background: '#4ade80', borderRadius: '2px', transition: 'width 0.25s linear' }} />
              </div>
            </div>

            {/* Controls — expand on hover */}
            <div style={{ maxHeight: hovering ? '120px' : '0', opacity: hovering ? 1 : 0, overflow: 'hidden', transition: 'max-height 0.25s ease, opacity 0.15s ease' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', padding: '10px 13px 10px', color: '#555' }}>
                <VolumeIcon size={12} />
                <input
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 13px 13px' }}>
      ...
      <footer style={{
        ...
        borderTop: '0.5px solid #1a1a1a', padding: '14px 48px',
        display: 'flex', justifyContent: 'center', alignItems: 'center',
        gap: '24px', fontSize: '12px', color: '#666', background: '#0a0a0a', flexShrink: 0,
      }}>
        <span>{utcTime}</span>
        ...
        {mounted && (
          <button
            onClick={() => setRadioOpen(prev => !prev)}
            title={radioOpen ? 'Close radio' : 'Open radio'}
            style={{
              position: 'absolute', right: '24px',
              background: 'none', border: 'none', cursor: 'pointer', padding: '6px',
              color: radioOpen || isPlaying ? '#4ade80' : '#555',
