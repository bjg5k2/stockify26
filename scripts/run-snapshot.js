...
async function fetchArtistWithRetry(artistId, token, retries = 3) {
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (res.status === 429) {
      ...
      console.log(`Rate limited on ${artistId}, waiting ${retryAfter}s...`)
      await new Promise(r => setTimeout(r, retryAfter * 1000))
      continue
    }
    if (!res.ok) {
      console.error(`Failed to fetch ${artistId}: ${res.status}`)
      return null
    }
    return res.json()
  }
  console.error(`Gave up on ${artistId} after ${retries} retries`)
  return null
}
...
async function main() {
  console.log('Starting snapshot run:', new Date().toISOString())
  ...
  if (!transactions || transactions.length === 0) {
    console.log('No artists to snapshot')
    return
  }

  const uniqueArtists = [...new Map(transactions.map(t => [t.artist_id, t])).values()]
  console.log(`Tracking ${uniqueArtists.length} unique artists`)
  ...
  for (const artistObj of uniqueArtists) {
    const artist = await fetchArtistWithRetry(artistObj.artist_id, token)
    if (!artist || !artist.id) {
      await sleep(50)
      continue
    }
    ...
    totalSnapshotted++
    if (totalSnapshotted % 50 === 0) console.log(`Progress: ${totalSnapshotted}/${uniqueArtists.length}`)
    await sleep(50)
  }

  console.log(`Snapshotted ${totalSnapshotted} artists`)
  ...
  for (const profile of profiles) {
    const { data: userHoldings } = await supabase
      .from('holdings').select('*').eq('user_id', profile.id)
  ...
  console.log(`Portfolio snapshots done for ${profiles.length} users`)

  // Dividends
  const isMonday = new Date().getDay() === 1

  const { data: allHoldings } = await supabase
    .from('holdings').select('id, user_id, artist_id, shares')
  ...
  const { data: todaySnaps } = await supabase
    .from('artist_snapshots').select('artist_id, artist_name, monthly_listeners, popularity')
  ...
  for (const profile of profiles) {
    ...
    await supabase.from('profiles').update({ credits: profile.credits + totalDividend }).eq('id', profile.id)
    await supabase.from('transactions').insert({
      user_id: profile.id, artist_id: null, artist_name: 'Dividends',
      type: 'dividend', shares: 0, price_per_share: 0, total: totalDividend,
    })

    const breakdownText = dividendBreakdown.map(d => `${d.artist_name} (+${Math.round(d.amount) || '<1'} CR)`).join(', ')
    ...
    await supabase.from('notifications').insert({
      user_id: profile.id, type: 'dividend', title: notifTitle,
  ...
  console.log(`Dividends distributed: ${totalDividendsPaid} CR to ${usersDividendedCount} users`)
  ...
  for (const alert of allAlerts || []) {
    ...
    let fired = false, notifTitle = '', notifBody = ''
    ...
    if (fired) {
      await supabase.from('notifications').insert({
        user_id: alert.user_id, type: 'price_alert',
  ...
  console.log(`Price alerts processed: ${alertsFired} fired`)

  // Morning market summary
  const { data: yesterdayArtistSnaps } = await supabase
    .from('artist_snapshots').select('artist_id, monthly_listeners, popularity')
  ...
  const { data: todayPortfolioSnaps } = await supabase
    .from('portfolio_snapshots').select('user_id, total_value, credits')
    .eq('snapshot_date', today)
  ...
  for (const profile of profiles) {
    ...
    let biggestMover = null, biggestMovePct = 0
    for (const h of userHoldings) {
      const artistData = snapMap[h.artist_id]
      if (!artistData) continue
    ...
    await supabase.from('notifications').insert({
      user_id: profile.id, type: 'market_summary',
      title: `📊 Market Open — ${new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`,
      body: bodyParts.join('\n'), read: false,
    })
  }

  console.log('Market summaries sent')
  console.log('Snapshot run complete:', new Date().toISOString())
}
